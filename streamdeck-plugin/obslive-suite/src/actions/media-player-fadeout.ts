...
export class MediaPlayerFadeout extends MediaPlayerBase {
	override async onKeyDown(ev: KeyDownEvent<MediaPlayerActionSettings>): Promise<void> {
		...
		try {
			await this.sendCommand(driverId, "fadeout");
			await ev.action.showOk();
		} catch (error) {
			console.error("[MediaPlayer Fadeout] Failed:", error);
	...
	protected override async updateButton(
		...
		await actionInstance.setImage(generateFadeoutIcon(bg, accent));
	}
}
